...
export default function DynamicWuxingPoster({issue,item,draws=[],mode='wuxing'}:{issue:string;item:Method;draws?:Draw[];mode?:'wuxing'|'jiaye'}){
  ...
  return <section className={`dynamic-poster ${mode}`} aria-label={`${issue}期${item.label}动态公式图`}>
    ...
    <div className="wuxing-formula-note"><b>公式算法</b>{item.branches.map((branch,index)=><span key={`${branch.name}-${index}`}>{branch.calculation||branch.name}</span>)}{item.verification&&<em className={item.verification.hit?'hit':'miss'}>{item.verification.hit?'准':'错'} · 特码{item.verification.actualNumber} {item.verification.actualElement}</em>}</div>
    </div>
