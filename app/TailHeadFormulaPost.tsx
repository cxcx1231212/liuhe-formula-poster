...
export default async function TailHeadFormulaPost({type,issue,method,kind}:{type:LotteryType;issue:string;method:string;kind:'tail'|'head'}){
  ...
  const sourceDraw=drawByPeriod.get(requestedIssue-1);if(!sourceDraw)return <ArchivedFormulaPost type={type} path={`/posts/${kind}/${issue}/${method}`} backHref={`/?type=${type}#board-${board}公式`} backLabel={`返回${board}板块`}/>;
