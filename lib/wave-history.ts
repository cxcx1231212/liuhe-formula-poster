...
export function buildWavePosterItem(method:WaveMethod,draws:ZodiacDraw[],requestedIssue:number){
  const ordered=draws.slice().sort((a,b)=>a.period-b.period);const evaluate=(draw:ZodiacDraw)=>{const raw=method.operation==='subtract'?baseValue(draw,method.baseName)-method.amount:baseValue(draw,method.baseName)+method.amount;const result=wrap(raw),prediction=wave(result),symbol=method.operation==='subtract'?'−':'+';return {prediction,calculation:`${baseExpression(draw,method.baseName)}${symbol}${method.amount}=${String(result).padStart(2,'0')}属${prediction}`};};
  ...
  const forecastSource=ordered.find(d=>d.period===requestedIssue-1)||ordered.at(-1)!;const forecast=evaluate(forecastSource);return {label:method.label,sourceKey:method.sourceKey,next:[forecast.prediction],recentStreak:0,recent30Hits:0,branches:[{name:method.name,next:forecast.prediction,calculation:forecast.calculation,sourcePositions:sourcePositions(method.baseName)}],history:histories.slice(-5),formulaId:method.rank};
}
