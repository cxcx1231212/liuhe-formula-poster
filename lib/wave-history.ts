...
const baseExpression=(d:ZodiacDraw,b:string)=>{if(b==='期数合数'){const digits=String(Math.abs(d.period)).split('');return `${d.period}期：${digits.join('＋')}＝${digitSum(d.period)}`;}const p=b.match(/^(平[1-6]码|特码)(合数|尾数)?([＋－])(平[1-6]码|特码)(合数|尾数)?$/);if(p)return `${String(converted(d,p[1],p[2])).padStart(2,'0')}${p[3]}${String(converted(d,p[4],p[5])).padStart(2,'0')}`;const s=b.match(/^(平[1-6]码|特码)(合数|尾数)?$/);return s?String(converted(d,s[1],s[2])).padStart(2,'0'):String(baseValue(d,b));};
