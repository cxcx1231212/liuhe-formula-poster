...
export default function TailHeadFormulaPost({type,issue,method,kind}:{type:LotteryType;issue:string;method:string;kind:'tail'|'head'}){
  ...
  const classify=(value:number)=>kind==='tail'?value%10:Math.floor(value/10);
  const unit=kind==='tail'?'尾':'头';
  const history=allDraws.slice(0,-1).map((source,position)=>{const target=allDraws[position+1];if(target.period!==source.period+1)return null;const prediction=evaluate(methodItem.spec as Spec,source);const result=classify(prediction.number),actual=classify(Number(target.numbers[6].number)),hit=result===actual;return {sourcePeriod:source.period,targetPeriod:target.period,branches:[{name:methodItem.name,calculation:`${prediction.calculation} 为${result}${unit}`,result:String(result),sourcePositions:prediction.sourcePositions,targetPositions:hit?[7]:[]}],actualNumber:target.numbers[6].number,actualAnimal:target.numbers[6].animal,actualElement:target.numbers[6].element,hit,targetPositions:hit?[7]:[]};}).filter(Boolean) as any[];
