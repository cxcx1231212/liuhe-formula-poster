...
const edgeEntry = {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === '/_entry/key') return protectedResponse(keyResponse(key));
      ...
      if (plainType.toLowerCase().includes('text/html') && response.status === 200) {
        if (!url.searchParams.has('__formula_payload')) return protectedResponse(encryptedPageShell(url));
        return protectedResponse(await encryptResponse(response, key, plainType, true));
      }
      if (/^application\/json\b|[+]json\b|^text\/x-component\b/i.test(plainType) || url.pathname.endsWith('.json')) {
        return protectedResponse(await encryptResponse(response, key, plainType || 'application/json; charset=utf-8'));
      }
