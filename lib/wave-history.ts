...
export function buildWavePosterItem(method:WaveMethod,draws:ZodiacDraw[],requestedIssue:number){
  const ordered=draws.slice().sort((a,b)=>a.period-b.period);const evaluate=(draw:ZodiacDraw)=>{const p=draw.period;const alternating=method.operation==='alternate_add_subtract'?(p%2?1:-1):method.operation==='double_alternate_add_subtract'?((Math.floor((p-1)/2)%2===0)?1:-1):method.operation==='triple_alternate_add_subtract'?((Math.floor((p-1)/3)%2===0)?1:-1):0;const base=baseValue(draw,method.baseName),delta=method.operation==='asymmetric_alternate'?(p%2?Math.floor(method.amount/100):-(method.amount%100)):method.operation==='cyclic_step'?((p-1)%3+1)*method.amount:(method.operation==='subtract'?-method.amount:(alternating||1)*method.amount);const raw=base+delta,prediction=wave(raw),symbol=delta<0?'−':'+';const baseText=baseExpression(draw,method.baseName),prefix=method.baseName==='期数合数'?`${baseText}；${base}`:baseText;return {prediction,calculation:`${prefix}${symbol}${Math.abs(delta)}=${cycle49Text(raw)}属${prediction}${cycle49(raw)!==raw?CYCLE49_NOTE:''}`};};
  const histories=[];for(let i=0;i<ordered.length-1;i++){const source=ordered[i],target=ordered[i+1];if(target.period>requestedIssue)continue;const answer=evaluate(source),actualNumber=Number(target.numbers[6].number),hit=answer.prediction===wave(actualNumber);histories.push({sourcePeriod:source.period,targetPeriod:target.period,branches:[{name:method.name,sourcePositions:explainSource(source,method.baseName).positions,calculation:answer.calculation,result:answer.prediction,targetPositions:hit?[7]:[]}],actualNumber:target.numbers[6].number,actualAnimal:target.numbers[6].animal,actualElement:target.numbers[6].element,hit,targetPositions:hit?[7]:[]});}
