...
def main():
    ...
    header = {"lotteryType": args.type, "year": args.year, "issue": issue, "draws": draws}
    path = output_dir / f"type-{args.type}-{issue:03d}-manifest.json"
    write_manifest(path, header, (("s", singles), ("d", pairs)), lambda rank, item: manifest_method(rank, item, records, history_records, element_map))
