...
export function makeHomeBoardPost(type:string,board:string,category:string,issue:number,m:Record<string,unknown>,index:number):BoardPost{
 ...
 if(board==='fushi'){const label:Record<string,string>={'22':'二中二','33':'三中三','2x':'二连肖','3x':'三连肖'};return {issue:i,href:`/posts/fushi/${category}/${issue}/${rank}${q}`,title:`${postAuthor(type,`fushi${category}` as never,index)}${category==='22'?'二中二【八码复式】':category==='33'?'三中三【十码复式】':`【${label[category]}】`}${fushiS[index%fushiS.length]}`};}
