import {cycle49} from './number-cycle';
...
const selectBranches=(method:FushiMethod,source:FushiDraw,targetPeriod:number,kind:string)=>{
  ...
  const original=method.branches.map(branch=>{const raw=evaluate(source,branch),number=kind==='number'?cycle49(raw.number):raw.number;return {...raw,number,calculation:kind==='number'&&number!==raw.number?`${raw.calculation.replace(/属[^属]+$/,'')}→回绕${String(number).padStart(2,'0')}`:raw.calculation,name:branch.name};});
  ...
  const add=(name:string)=>{const raw=evaluate(source,name),number=cycle49(raw.number),value={...raw,number,calculation:number!==raw.number?`${raw.calculation.replace(/属[^属]+$/,'')}→回绕${String(number).padStart(2,'0')}`:raw.calculation,name};if(!seen.has(value.number)){seen.add(value.number);result.push(value);}};
