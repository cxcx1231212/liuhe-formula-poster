...
export default function LiveDraw({ initial, type }: { initial: LatestLottery; type: LotteryType }) {
  ...
  </div>{showPlayer && <div className="live-player"><header><div><i></i><strong>开奖直播</strong><span>LIVE</span></div><button type="button" onClick={() => setShowPlayer(false)}>收起 ×</button></header><div className="live-screen"><div className="live-video-logo" aria-label="六合公式库"><img src="/favicon.svg" alt=""/><span><strong>六合公式库</strong><small>GS88888.COM</small></span></div>{latest.videoUrlForH5 || latest.videoUrl ? <video ref={videoRef} controls autoPlay muted playsInline/> : <p>当前暂无直播信号</p>}</div></div>}</section>;
}
