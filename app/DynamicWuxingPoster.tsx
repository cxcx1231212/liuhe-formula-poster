...
export default function DynamicWuxingPoster({issue,item,draws=[],mode='wuxing',lotteryName='澳门六合彩'}:{issue:string;item:Method;draws?:Draw[];mode?:'wuxing'|'jiaye';lotteryName?:string}){
  ...
  return <section className={`dynamic-poster ${mode}`} aria-label={`${issue}期${item.label}动态公式图`}>
    ...
    {draws.length>0&&<div className="wuxing-line-board"><div className="wuxing-board-head"><b>期号</b>{['平1码','平2码','平3码','平4码','平5码','平6码','特码'].map(label=><b key={label}>{label}</b>)}</div>{!item.verification&&<div className="wuxing-forecast-row"><strong>{issue}期{mode!=='jiaye'&&<small>下期预测</small>}</strong><div>{mode!=='jiaye'&&<span>五行参考</span>}{mode!=='jiaye'&&item.next.map(value=><b key={value} style={{backgroundColor:colors[value]||'#9b772e'}}>{value}</b>)}</div></div>}{orderedDraws.map((draw,drawIndex)=><div className="wuxing-board-row" key={draw.period}><strong>{draw.displayPeriod||`${String(draw.period).padStart(3,'0')}期`}<small>{draw.date||''}</small></strong>{draw.numbers.map((value,index)=>{const label=index===6?'特码':`平${index+1}码`;const isPredictionSource=!item.verification&&drawIndex===0&&sourcePositions.has(label);const isSource=isPredictionSource||(sourcePositions.has(label)&&validations.some(entry=>entry.sourcePeriod===draw.period));const isTarget=index===6&&validations.some(entry=>entry.targetPeriod===draw.period);return <span className={`${isSource?'picked ':''}${isTarget?'target ':''}`} key={`${draw.period}-${index}`}><b>{value.number}</b><small>{value.animal} · {value.element}</small></span>})}</div>)}<svg className="wuxing-board-lines" viewBox={`0 0 1000 ${boardOffset+orderedDraws.length*104}`} preserveAspectRatio="none" aria-hidden="true"><defs><marker id="wuxing-arrow" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z" fill="#cf2f32"/></marker></defs>{!item.verification&&orderedDraws[0]&&item.branches[0]?.sourcePositions?.map(position=><path className="prediction-arrow" key={`prediction-${position}`} d={`M ${columnX(position)} ${rowY(orderedDraws[0].period)-18} C ${columnX(position)} 190, 350 170, 430 151`} markerEnd="url(#wuxing-arrow)"/>)}{validations.map((entry,index)=>{const positions=item.branches[index%item.branches.length]?.sourcePositions||item.branches[0]?.sourcePositions||[];const sy=rowY(entry.sourcePeriod);const ty=rowY(entry.targetPeriod);const tx=columnX(6);const joinX=tx-56;const joinY=(sy+ty)/2;return <g key={`${entry.sourcePeriod}-${entry.targetPeriod}`}>{positions.map(position=><path key={position} d={`M ${columnX(position)} ${sy-18} C ${columnX(position)} ${joinY+18}, ${joinX-28} ${joinY+12}, ${joinX} ${joinY}`} />)}<path d={`M ${joinX} ${joinY} C ${joinX+28} ${joinY-8}, ${tx} ${ty+30}, ${tx} ${ty+7}`} markerEnd="url(#wuxing-arrow)"/><foreignObject x="432" y={joinY-18} width="425" height="42"><div className="wuxing-line-label"><span>{entry.branches.map(branch=>branch.calculation).join('　')}</span><i className={entry.hit?'hit':'miss'}>{entry.hit?'准':'错'}</i></div></foreignObject></g>})}</svg></div>}
    {!item.verification&&<div className="wuxing-formula-note"><b>公式算法</b>{item.branches.map((branch,index)=><span key={`${branch.name}-${index}`}>{branch.calculation||branch.name}</span>)}{mode==='jiaye'&&item.next.map(value=><strong className="prediction-result" style={{backgroundColor:colors[value]||'#9b772e'}} key={value}>{value}</strong>)}</div>}
    </div>
