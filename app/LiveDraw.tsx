...
export default function LiveDraw({ initial, type }: { initial: LatestLottery; type: LotteryType }) {
  ...
  </div>{showPlayer && <div className="live-player"><header><div><i></i><strong>开奖直播</strong><span>LIVE</span></div><button type="button" onClick={() => setShowPlayer(false)}>收起 ×</button></header><div className="live-screen"><img className="live-video-logo" src="/live-logo.jpg" alt="六合直播 6H.COM"/>{latest.videoUrlForH5 || latest.videoUrl ? <video ref={videoRef} controls autoPlay muted playsInline/> : <p>当前暂无直播信号</p>}</div></div>}</section>;
}
