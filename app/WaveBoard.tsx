...
export default function WaveBoard({type,issue,methods}:{type:string;issue:number;methods:Method[]}){
  return <><div className="pingte-count">特波连准榜 · 当前共 {methods.length} 条公式</div><BoardPostList posts={methods.map((method,index)=>({href:`/posts/wave/${issue}/${method.rank}?type=${type}`,issue:`${issue}期`,title:`${postAuthor(type,'wave',index)}【特码波色】${slogans[index%slogans.length]}`}))}/></>;
}
