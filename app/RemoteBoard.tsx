...
export default function RemoteBoard({type,board,version,categories=[{key:'',label:''}]}:{type:string;board:string;version:number;categories?:Category[]}){
  ...
  return <div ref={root} className="remote-board">
    ...
    {error?<div className="pingte-empty"><strong>读取失败</strong><button onClick={()=>{setVisible(false);requestAnimationFrame(()=>setVisible(true))}}>重新加载</button></div>:data?<><div className="board-titles">{data.posts.map(post=><a href={post.href} target="_blank" rel="noopener noreferrer" key={post.href}><span>{post.issue}</span><h3>{post.title}</h3><i>›</i></a>)}</div>{data.pages>1&&<nav className="board-pagination" aria-label="帖子分页"><button disabled={page===1} onClick={()=>setPage(v=>v-1)}>上一页</button>{pageNumbers.map(v=><button className={page===v?'active':''} onClick={()=>setPage(v)} key={v}>{v}</button>)}<button disabled={page===data.pages} onClick={()=>setPage(v=>v+1)}>下一页</button></nav>}</>:<div className="board-list-loading" aria-hidden="true"/>}
  </div>;
}
