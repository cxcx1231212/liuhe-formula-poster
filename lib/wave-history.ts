...
export function buildWavePosterItem(method:WaveMethod,draws:ZodiacDraw[],requestedIssue:number){
  const ordered=draws.slice().sort((a,b)=>a.period-b.period);const evaluate=(draw:ZodiacDraw)=>{const direction=method.operation==='subtract'||(method.operation==='alternate_add_subtract'&&draw.period%2===0)?-1:1;const raw=baseValue(draw,method.baseName)+direction*method.amount;const result=wrap(raw),prediction=wave(result),symbol=direction<0?'−':'+';return {prediction,calculation:`${baseExpression(draw,method.baseName)}${symbol}${method.amount}=${String(result).padStart(2,'0')}属${prediction}`};};
