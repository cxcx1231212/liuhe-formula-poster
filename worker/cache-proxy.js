...
const INDEX_KV_TTL = 600;
const HISTORY_KV_TTL = 60 * 60 * 24 * 90;

function isCacheableDocument(request, url) {
  const isPage = url.pathname === '/' || POST_PATH.test(url.pathname);
  return request.method === 'GET' && isPage &&
    !request.headers.has('RSC') && !request.headers.has('Next-Router-Prefetch') &&
    (request.headers.get('accept') || '').includes('text/html');
}

function cacheKey(url) { return new Request(url.toString(), { method: 'GET' }); }
function globalKey(url) { return 'html:' + url.pathname + url.search; }
function kindOf(headers) { return headers.get('x-formula-edge-cache-kind') || headers.get('x-formula-cache-kind') || 'current'; }
function ttlFor(kind) { return kind === 'history' ? HISTORY_TTL : kind === 'index' ? INDEX_TTL : CURRENT_TTL; }
function kvTtlFor(kind) { return kind === 'history' ? HISTORY_KV_TTL : kind === 'index' ? INDEX_KV_TTL : CURRENT_KV_TTL; }

function makeResponse(body, sourceHeaders, edgeState, globalState, kind) {
  const headers = new Headers(sourceHeaders);
  headers.delete('set-cookie');
  headers.delete('vary');
  headers.set('cache-control', 'public, max-age=0, s-maxage=' + ttlFor(kind));
...
async function storeGlobal(env, url, response, kind) {
  ...
  await env.HTML_CACHE.put(globalKey(url), html, {
    expirationTtl: kvTtlFor(kind),
    metadata: { kind, contentType: response.headers.get('content-type') || 'text/html; charset=utf-8' },
  });
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (cached) {
      ...
      if (cached.headers.get('x-formula-global-cache') !== 'HIT') ctx.waitUntil(storeGlobal(env, url, cached.clone(), kind));
      return makeResponse(cached.body, cached.headers, 'HIT', cached.headers.get('x-formula-global-cache') || 'FILLING', kind);
    }

    const stored = await env.HTML_CACHE.getWithMetadata(globalKey(url), { type: 'text', cacheTtl: 60 });
    if (stored.value !== null) {
      const kind = stored.metadata?.kind || (url.pathname === '/' ? 'index' : 'history');
    ...
    const kind = url.pathname === '/' ? 'index' : kindOf(origin.headers);
