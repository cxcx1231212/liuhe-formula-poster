...
class Scorer:
    ...
    def score(self,board,group,item):
        ...
        for i,(source,target) in enumerate(self.pairs):
            raw_ns={p[i] for p in predictions};ns={cycle49(x) for x in raw_ns} if board=='tema' or (board=='fushi' and group in ('22','33')) or (board=='kill' and group in ('code','wave','tail','head')) else raw_ns;balls=target['numbers'];special=balls[6];n=int(special['number'])
