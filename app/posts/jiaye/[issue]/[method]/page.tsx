...
export default async function JiayePost({params,searchParams}:{params:Promise<{issue:string;method:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <main className="post-page">
    ...
    <article className="detail pingte-detail">
      <div className="detail-topbar"><a className="detail-back" href={`/?type=${type}#board-家野公式`}><i>←</i><span><small>BACK TO INDEX</small><strong>返回家野板块</strong></span></a>{previousIssue?<a className="dynamic-poster-badge" href={`/posts/jiaye/${previousIssue}/${method}?type=${type}`}>期数切换 · {previousIssue}期</a>:<span className="dynamic-poster-badge">暂无上期</span>}</div>
