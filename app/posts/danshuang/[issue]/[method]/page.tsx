...
export default async function DanshuangPost({params,searchParams}:{params:Promise<{issue:string;method:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  if(!item||!Number.isFinite(requestedIssue)||requestedIssue<minimumIssue||requestedIssue>currentIssue)return <ArchivedFormulaPost type={type} path={`/posts/danshuang/${issue}/${method}`} backHref={`/_entry/home?type=${type}#board-单双公式`} backLabel="返回单双板块"/>;
