...
export default async function SizePost({params,searchParams}:{params:Promise<{issue:string;method:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  if(!methodItem||requestedIssue<2||requestedIssue>currentIssue)return <ArchivedFormulaPost type={type} path={`/posts/size/${issue}/${method}`} backHref={`/_entry/home?type=${type}#board-大小公式`} backLabel="返回大小板块"/>;
  ...
  const explain=(prediction:ReturnType<typeof evaluate>)=>{const number=cycle49(prediction.number),cycle=number===prediction.number?'':`；${cycle49Text(prediction.number)}${CYCLE49_NOTE}`;return `${prediction.calculation}${cycle}，${String(number).padStart(2,'0')}为${sizeOf(number)}`;};
  const history=allDraws.slice(0,-1).map((source,position)=>{const target=allDraws[position+1];if(target.period!==source.period+1)return null;const prediction=evaluate(methodItem.spec as Spec,source),number=cycle49(prediction.number);const actual=Number(target.numbers[6].number);const hit=sizeOf(number)===sizeOf(actual);return {sourcePeriod:source.period,targetPeriod:target.period,branches:[{name:methodItem.name,calculation:explain(prediction),result:sizeOf(number),sourcePositions:prediction.sourcePositions,targetPositions:hit?[7]:[]}],actualNumber:target.numbers[6].number,actualAnimal:target.numbers[6].animal,actualElement:target.numbers[6].element,hit,targetPositions:hit?[7]:[]};}).filter(Boolean) as any[];
  const sourceDraw=drawByPeriod.get(requestedIssue-1);if(!sourceDraw)return <ArchivedFormulaPost type={type} path={`/posts/size/${issue}/${method}`} backHref={`/_entry/home?type=${type}#board-大小公式`} backLabel="返回大小板块"/>;
  ...
  const result=sizeOf(cycle49(prediction.number));const item={...methodItem,next:[result],branches:[{name:methodItem.name,next:result,calculation:explain(prediction),sourcePositions:prediction.sourcePositions}],history:visibleHistory,verification:verification?{hit:verification.hit,actualNumber:verification.actualNumber,actualAnimal:verification.actualAnimal,actualElement:verification.actualElement}:undefined};
