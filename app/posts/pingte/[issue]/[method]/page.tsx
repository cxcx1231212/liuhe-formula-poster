...
function calculation(name:string,draw:Draw|undefined,result:number){
  ...
  if(match){const value=values[match[1]==='特码码'?6:Number(match[2])-1],mode=match[3],base=mode==='合数'?digits(value):mode==='尾数'?value%10:value,plus=Number(match[4]),minus=Number(match[5]),amount=draw.period%2?plus:minus,direction=draw.period%2?1:-1;return `${String(value).padStart(2,'0')}${mode==='合数'?`合${base}`:mode==='尾数'?`尾${base}`:''}${direction>0?'＋':'－'}${amount}＝${base+direction*amount}`}
  match=name.match(/^(平([1-6])码|特码码)(固定|合数|尾数)循环步长(\d+)$/);if(match){const value=values[match[1]==='特码码'?6:Number(match[2])-1],mode=match[3],base=mode==='合数'?digits(value):mode==='尾数'?value%10:value,step=((draw.period-1)%3+1)*Number(match[4]);return `${String(value).padStart(2,'0')}＋${step}＝${base+step}`}
  ...
  if(match){const value=values[Number(match[1])-1],amount=Number(match[3]);return `${String(value).padStart(2,'0')}${match[2]==='加'?'＋':'－'}${amount}＝${value+(match[2]==='加'?amount:-amount)}`}
  match=name.match(/平(\d)码尾数(加|减)(\d+)/);if(match){const value=values[Number(match[1])-1],tail=value%10,amount=Number(match[3]);return `${String(value).padStart(2,'0')}尾${tail}${match[2]==='加'?'＋':'－'}${amount}＝${tail+(match[2]==='加'?amount:-amount)}`}
