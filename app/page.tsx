...
export default async function Home({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <main>
    ...
    <section className="board-sections" id="boards">{boards.map((board,i)=><section className="board-section" key={board.name} id={`board-${board.name}`}>
      ...
      {board.name==='平特公式'?<PingteBoard type={type} issue={one.issue} oneMethods={one.methods} twoMethods={two.methods}/>:board.name==='特码公式'?<TemaBoard type={type} issue={tema.issue} oneMethods={temaOne.qualifiedMethods} groups={tema.groups}/>:board.name==='生肖公式'?<ZodiacBoard type={type} issue={zodiac.issue} groups={zodiac.groups}/>:board.name==='复式公式'?<FushiBoard type={type} issue={fushi.issue} groups={fushi.groups}/>:board.name==='单双公式'?<DanshuangBoard type={type} issue={ds.issue} methods={ds.methods}/>:board.name==='波色公式'?<WaveBoard type={type} issue={wave.issue} methods={wave.methods}/>:board.name==='五行公式'?<WuxingBoard type={type} issue={wx.issue} methods={wx.methods}/>:board.name==='家野公式'?<JiayeBoard type={type} issue={jy.issue} methods={jy.methods}/>:board.name==='绝杀公式'?<KillBoard type={type} issue={kill.issue} groups={kill.groups}/>:board.name==='大小公式'?<SizeBoard type={type} issue={size.issue} methods={size.methods}/>:board.name==='尾数公式'?<TailHeadBoard type={type} issue={tail.issue} methods={tail.methods} kind="tail"/>:board.name==='头数公式'?<TailHeadBoard type={type} issue={head.issue} methods={head.methods} kind="head"/>:null}
    </section>)}</section>
