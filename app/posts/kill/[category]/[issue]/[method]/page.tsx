...
export default async function KillPost({params,searchParams}:{params:Promise<{category:string;issue:string;method:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  const {category,issue,method}=await params;const type=requestedLotteryType(await searchParams);const manifest=(await formulaManifests.kill[type]);const group=(manifest.groups as any)[category];const index=group?.methods.findIndex((value:any)=>value.rank===method)??-1;const item=index>=0?group.methods[index]:null;
