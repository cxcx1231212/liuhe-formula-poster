...
import {formulaManifests} from '@/lib/formula-manifests';
import type {LotteryType} from '@/lib/lottery';
...
export default async function TailHeadFormulaPost({type,issue,method,kind}:{type:LotteryType;issue:string;method:string;kind:'tail'|'head'}){
  ...
  if(!methodItem||requestedIssue<2||requestedIssue>currentIssue)return <ArchivedFormulaPost type={type} path={`/posts/${kind}/${issue}/${method}`} backHref={`/_entry/home?type=${type}#board-${board}公式`} backLabel={`返回${board}板块`}/>;
  ...
  const classify=(value:number)=>{const number=cycle49(value);return kind==='tail'?number%10:Math.floor(number/10);};
  const explain=(prediction:ReturnType<typeof evaluate>)=>{const number=cycle49(prediction.number),result=classify(prediction.number),cycle=number===prediction.number?'':`；${cycle49Text(prediction.number)}${CYCLE49_NOTE}`;return `${prediction.calculation}${cycle}，${String(number).padStart(2,'0')}为${result}${kind==='tail'?'尾':'头'}`;};
  const history=allDraws.slice(0,-1).map((source,position)=>{const target=allDraws[position+1];if(target.period!==source.period+1)return null;const prediction=evaluate(methodItem.spec as Spec,source);const result=classify(prediction.number),actual=classify(Number(target.numbers[6].number)),hit=result===actual;return {sourcePeriod:source.period,targetPeriod:target.period,branches:[{name:methodItem.name,calculation:explain(prediction),result:String(result),sourcePositions:prediction.sourcePositions,targetPositions:hit?[7]:[]}],actualNumber:target.numbers[6].number,actualAnimal:target.numbers[6].animal,actualElement:target.numbers[6].element,hit,targetPositions:hit?[7]:[]};}).filter(Boolean) as any[];
  const sourceDraw=drawByPeriod.get(requestedIssue-1);if(!sourceDraw)return <ArchivedFormulaPost type={type} path={`/posts/${kind}/${issue}/${method}`} backHref={`/_entry/home?type=${type}#board-${board}公式`} backLabel={`返回${board}板块`}/>;
  const prediction=evaluate(methodItem.spec as Spec,sourceDraw),result=classify(prediction.number);const verification=requestedIssue<currentIssue?history.find(entry=>entry.targetPeriod===requestedIssue):undefined;const visibleHistory=history.filter(entry=>entry.targetPeriod<=requestedIssue).slice(-5);
  const item={...methodItem,next:[String(result)],branches:[{name:methodItem.name,next:String(result),calculation:explain(prediction),sourcePositions:prediction.sourcePositions}],history:visibleHistory,verification:verification?{hit:verification.hit,actualNumber:verification.actualNumber,actualAnimal:verification.actualAnimal,actualElement:verification.actualElement}:undefined};
  ...
  return <DynamicSimpleFormulaPost type={type} issue={String(posterIssue)} navigationIssue={issue} method={method} item={methodItem} draws={draws} board={board} hash={`${board}公式`} basePath={`/posts/${kind}`} index={index} total={manifest.methods.length} posterItemOverride={item} periodNav={<IssueScroller issues={availableIssues} current={requestedIssue} basePath={`/posts/${kind}`} method={method} type={String(type)}/>} note={`按上期开奖推算下期${board} · 仅供娱乐参考`}/>;
}
